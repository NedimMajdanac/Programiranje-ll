...
ostream& operator<<(ostream& COUT, Karakteristike obj) {
	const char* ispis[] = { "NARUDZBA", "KVALITET", "PAKOVANJE", "ISPORUKA" };
	COUT << ispis[obj];
	return COUT;
}
...
public:
	...
	void DodajKupovinu(float bodovi, ZadovoljstvoKupca zk) {
		_kupovine->AddElement(bodovi, zk);
		if (bodovi >= 10)
			_bodovi.push_back(bodovi / 10);
		if (_bodovi.size() != 0) {
