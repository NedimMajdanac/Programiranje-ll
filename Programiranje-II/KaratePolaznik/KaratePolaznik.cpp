...
ostream& operator<<(ostream& COUT, Pojas obj) {
    const char* ispis[] = { "BIJELI", "ZUTI", "NARANDZASTI", "ZELENI", "PLAVI", "SMEDJI", "CRNI" };
    COUT << ispis[obj];
    return COUT;
}
...
public:
    ...
    bool AddOcjena(int ocjena, Datum datum) {
        ...
        if (trenutno != 0 && GetRazliku(_ocjene->getElement2(trenutno - 1), datum) < 3)
            return false;
...
class KaratePolaznik:public Korisnik {
    ...
    void sendMail(Tehnika tehnika,Pojas p) {
        cout << "FROM:info@karate.ba" << endl;
        cout << "TO: " << GetEmail() << endl;
        cout << "Postovani " << GetImePrezime() << ", evidentirana vam je thenika " << tehnika.GetNaziv() << " za " << p << "pojas. Dosadasnji uspjeh(prosjek ocjena) na pojasu " << p << "iznosi " << tehnika.getProsjekTehnike() << ", a ukupni uspjeh(prosjek ocjena) na svim pojasevima iznosi" << 3 << endl;
        cout << "Pozdrav.\nKARATE Team." << endl;
    }
public:
    void info() {}
    ...
    bool AddTehniku(Pojas pojas, Tehnika tehnika) {
        for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
        {
            if (pojas > _polozeniPojasevi[i].GetPojas()) {
                for (size_t j = 0; j < _polozeniPojasevi[i].GetTehnike().size(); j++)
                {
                    if (_polozeniPojasevi[i].GetTehnike()[j]->getProsjekTehnike() < 3.5 || _polozeniPojasevi[i].GetTehnike().size() < 3)
                        return false;
                }
            }
            if (_polozeniPojasevi[i].GetPojas() == pojas) {
                ...
                thread t1(&KaratePolaznik::sendMail, this, tehnika, pojas);
                t1.join();
                return true;
            }
        }
