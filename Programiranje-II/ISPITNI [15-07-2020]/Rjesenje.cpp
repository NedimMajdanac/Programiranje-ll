...
bool ValidirajLozinku(string lozinka) {
    return regex_search(lozinka, regex("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[!@#$%^&*()_+\\-={}:\"|<>?,./]).{7,}$"));
}
...
public:
    ...
    Kolekcija(const Kolekcija& obj) {
        ...
        for (size_t i = 0; i < getTrenutno(); i++)
        {
            ...
            _elementi2[i] = new T2(*obj._elementi1[i]);
        }
    }
    ...
    void AddElement(T1 el1, T2 el2) {
        if (_trenutno >= max)
            throw exception("VAN DOSEGA KOLEKCIJE");
        _elementi1[_trenutno] = new T1(el1);
        _elementi2[_trenutno] = new T2(el2);
        _trenutno++;
    }
    void RemoveAt(int lokacija) {
        for (size_t i = lokacija; i < getTrenutno()-1; i++)
        {
...
int getRazlikaDana(Datum d1,Datum d2) {
...
public:
    ...
    friend ostream& operator<< (ostream& COUT, const Tehnika& obj) {
        ...
        COUT << "Prosjek tehnike -> " << obj.getProsjekTehnike() << endl;
        return COUT;
    }
   ...
   */
    bool AddOcjena(int ocjena, Datum datum) {
        int trenunto = _ocjene->getTrenutno();
        if (trenunto != 0 && getRazlikaDana(_ocjene->getElement2(trenunto - 1), datum) < 3)
            return false;
...
public:
    ...
    friend ostream& operator<< (ostream& COUT, const Polaganje& obj) {
        COUT << ispisPojas[obj._pojas] << endl;
    ...
    float getProsjekPojasa()const {
        ...
        for (size_t i = 0; i < _polozeneTehnike.size(); i++)
        {
            sum += _polozeneTehnike[i]->getProsjekTehnike();
        }
...
class KaratePolaznik:public Korisnik {
    ...
    void sendMail(Pojas pojas, Tehnika tehnika) {
        m.lock();
        cout << crt;
        cout << "FROM:info@karate.ba" << endl;
        cout << "TO: " << GetEmail() << endl;
        cout << "Postovani " << GetImePrezime() << " evidentirana vam je " << "tehnika " << tehnika.GetNaziv() << "za " << ispisPojas[pojas] << " pojas.Dosadasnji uspjeh(prosjek ocjena)" << endl;
        ...
        cout << ", a ukupni uspjeh(prosjek ocjena) na svim pojasevima iznosi " << getMainProsjek() << endl;
        cout << "Pozdrav" << endl;
...
public:
    ...
    */
    ...
    bool checkSame(Pojas pojas, Tehnika tehnika) {
        for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
        {
            if (_polozeniPojasevi[i].GetPojas() == pojas) {
    ...
    float getMainProsjek()const {
        float sum = 0;
        if (_polozeniPojasevi.size() == 0)
            return sum;
        for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
        {
            sum += _polozeniPojasevi[i].getProsjekPojasa();
        }
    ...
    bool AddTehniku(Pojas pojas, Tehnika tehnika) {
        for (size_t i = 0; i < _polozeniPojasevi.size(); i++)
        {
            if (pojas > _polozeniPojasevi[i].GetPojas() &&
                (_polozeniPojasevi[i].GetTehnike().size() < 3 ||
                    _polozeniPojasevi[i].getProsjekPojasa() < 3.5))
                return false;
            if (_polozeniPojasevi[i].GetPojas() == pojas) {
                if (checkSame(pojas, tehnika))
                    return false;

                _polozeniPojasevi[i].GetTehnike().push_back(new Tehnika(tehnika));
                thread t1(&KaratePolaznik::sendMail, this, pojas, tehnika);
                t1.join();
                return true;
            }
        }
